...
import { fetchUserData } from "../Register/login.js";

// Initialize Stripe with your publishable key
const stripe = await loadStripe(
  "pk_test_51Qf9q7AnBfxoX5a3ubkQ9mbIGvB6FujzedMCkDo7AvQXnz9ZHZSbsrGP8P8oEWkMc6eckOGJthugfD0tk3Gljibw00uNOSK4YD"
);
...
document
  ...
  .addEventListener("click", async function () {
    ...
    const addressOptionSelected = document.querySelector(
      'input[name="address-option"]:checked'
    );
    const errorContainer = document.querySelector(
      ".footer-error .error-body-text"
    );
    ...
    try {
      ...
      // postData(
      //   { ...JSON.parse(authData.id), orders: existingOrders },
      //   JSON.parse(authData).id
      // );
      ...
      // localStorage.setItem('orders', JSON.stringify(existingOrders));
      setCookie("orders", existingOrders, 1, authType);
      // postData({ ...JSON.parse(authData), orders: existingOrders });
